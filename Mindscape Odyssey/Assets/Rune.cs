...
void Update()
{
    ...
    if (playerInside && !isActivated)
    {
        ...
        breathingIcon.position = Vector3.Lerp(breathingIcon.position, iconAboveRunePosition.position, 0.4f);
    }
    else if (!oneActive)
    {
        Debug.Log("Returning");
        
        breathingIcon.position = Vector3.Lerp(breathingIcon.position, iconStartPosition.position, 0.4f);
    }
}
