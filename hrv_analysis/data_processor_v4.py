# Changing the routes for game flags
...
log_file = "data_processor_log.txt"
...
def data_processor(ppg_data_queue, eda_data_queue, stop_event):
    ...
    anxious_calibration_state = 0  # 0->no signal, 1->recieved signal, 2->finished calibration
        ...
        try:
            ...
            if step_counter >= step_size:
                if len(buffer) >= window_size:
                    ...
                    expected_timestamp = (current_step + 5) * 5  # Assuming each step is 5 seconds - +5 because of initial window
                    ...
                    if current_step <= num_steps_skipped and current_step >= 1:
                        ...
                        if current_step == num_steps_skipped:
                            log_msg = f"The next {num_steps_for_baseline} will be used to calculate the calm baseline.\n\n"
                            add_log_entry(log_msg)
                    ...
                    # Check for anxious calibration signal
                        ...
                        if anxious_calibration_signal == 1:
                            ...
                            f"Actual Timestamp {actual_timestamp_minutes}min {actual_timestamp_seconds}sec\n")
                            add_log_entry(log_msg)
                            add_log_entry("Anxious calibration signal received. Starting anxious calibration in the next step.\n\n")
                        else:
                            # Continue calculating HRV without taking any actions
                            log_msg = (f"Segment {current_step} - "
                            f"Expected Timestamp {expected_timestamp_minutes}min {expected_timestamp_seconds}sec, "
                            f"Actual Timestamp {actual_timestamp_minutes}min {actual_timestamp_seconds}sec\n")
                            add_log_entry(log_msg)
                            add_log_entry(f"Segment skipped while waiting for anxious calibration signal.\n\n")
                            continue
                        
                    elif anxious_calibration_state == 1:
                        # Determine the ranges for skipping and baseline collection
                        start_skip_range = anxious_calibration_start_step
                        end_skip_range = start_skip_range + anxious_num_steps_skipped
                        end_baseline_range = end_skip_range + anxious_num_steps_for_baseline
                        
                        if current_step == anxious_calibration_start_step + 1:
                            log_msg = f"\nAnxious Calibration Starting...\n\n"
                            add_log_entry(log_msg)
                            log_msg = f"The next {anxious_num_steps_skipped} will be skipped\n\n"
                            add_log_entry(log_msg)

                        if current_step <= end_skip_range:
                            # Skipping steps for anxious calibration
                            add_log_entry(f"Segment {current_step} skipped during anxious calibration.\n")
                            log_msg = (f"Segment {current_step} skipped - "
                            f"Expected Timestamp {expected_timestamp_minutes}min {expected_timestamp_seconds}sec, "
                            f"Actual Timestamp {actual_timestamp_minutes}min {actual_timestamp_seconds}sec\n")
                            add_log_entry(log_msg)
                            ...
                            if current_step == end_skip_range:
                                log_msg = f"The next {anxious_num_steps_for_baseline} will be used to calculate the anxious baseline.\n\n"
                                add_log_entry(log_msg)
                            
                        elif current_step <= end_baseline_range:
                            # Collect baseline HRV values
                            anxious_baseline_hrv.append(current_hrv)
                            add_log_entry(f"Segment {current_step}: HRV during anxious calibration = {current_hrv}\n")
                            log_msg = (f"Segment {current_step} - "
                            f"Expected Timestamp {expected_timestamp_minutes}min {expected_timestamp_seconds}sec, "
                            f"Actual Timestamp {actual_timestamp_minutes}min {actual_timestamp_seconds}sec\n")
                            add_log_entry(log_msg)
